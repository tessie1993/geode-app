...
// Set in main(): the key's tint on every strand and mote.
float gKeyShift;
...
Strand strands(vec2 t, float count, float beadPulse, vec3 light) {
    ...
    vec3 col = pick < 1.0 ? ink(0.0) * vec3(0.85, 0.45, 0.5) : (pick < 2.0 ? ink(0.75) * vec3(0.85, 0.8, 0.35) : ink(0.20) * 0.9);
    col *= 0.15 + 0.8 * diff;
...
void main() {
    ...
    // swells the beads, a snare twists the strands, the hats twinkle the
    // dust, a buildup packs the beads, a drop clears the haze for a while,
    // the bar sways the light, the section spaces the rings, the key tints
    // it all. All slew-limited upstream.
    gKeyShift = 0.20 * (uKeyHue - 0.5) * uKeyStrength;
    float beadPulse = clamp(uBassSmooth * uBeatResponse, 0.0, 1.5) + touchFalloff(uv, 0.5) + 0.5 * uKick;
    ...
    float spin = travel * 0.12 + 0.35 * uSnare;
    vec3 light = normalize(vec3(0.35 + 0.20 * uRhythmLock * sin(uBarPhase * BV_TAU), 0.55, 0.75));

    float pack = 1.0 + 0.30 * uBuild;
    vec2 front = vec2(fract((a + 1.1 * depth + spin) / BV_TAU), depth * 3.0 * pack + travel * 0.8);
    vec2 back = vec2(fract((a - 0.8 * depth - spin * 0.6) / BV_TAU + 0.37), depth * 4.2 * pack + travel * 1.1);
    ...
    float hazeAmt = smoothstep(0.78 - 0.25 * uDrop, 0.12, r);
    float rings = 0.5 + 0.5 * sin(depth * (12.0 + 6.0 * uSectionPhase) - travel * 3.0);
    vec3 haze = ink(0.24) * (0.24 + 0.28 * rings) + ink(0.36) * 0.2;
    ...
    haze += mix(ink(0.5), ink(0.88), step(0.8, moteSeed)) * motes * (1.2 + 0.8 * uHat) * hazeAmt;
