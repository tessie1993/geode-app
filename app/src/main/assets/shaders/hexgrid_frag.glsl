...
// Hexagonal cells, each pulsing with its own frequency band.
...
// motion: uBarOsc -> phase-locked sweep across the tile edges (was beat*beatResponse), uMotionBright -> edge line weight
void main() {
    vec2 uv = view() * (5.0 + uMorph * 6.0);
    vec4 hc = hexCoords(uv + vec2(0.0, uTime * 0.4));
    float band = fract(sin(dot(hc.zw, vec2(12.9898, 78.233))) * 43758.5453);
    float e = aband(band) * (0.5 + uEnergy);
    float d = hexDist(hc.xy);
    float cell = smoothstep(0.5, 0.46, d);
    float edge = smoothstep(0.035 + 0.02 * uMotionBright, 0.0, abs(d - 0.45));
    vec3 col = pal(band) * (cell * e + edge * (0.25 + uTreble + uBarOsc * 0.3));
