...
// motion: uOrbit -> Julia c target drift (was a static bass/mid offset only), uHarmony -> brightness kick (was beat*beatResponse)
void main() {
    vec2 uv = view() * 1.4;
    ...
    float t = uTime * 0.15;
    vec2 c = vec2(-0.745 + 0.11 * cos(t) + uBass * 0.08 + uOrbit.x * 0.05,
                  0.186 + 0.11 * sin(t * 1.3) + uMid * 0.05 + uOrbit.y * 0.05);
    vec2 z = uv;
    float iter = 0.0;
    ...
    vec3 col = pal(f * (1.0 + uTreble)) * (0.25 + f * 1.2 + aband(f) * 0.8);
    col += clamp(uHarmony - 0.5, -0.5, 0.5) * 0.15;
