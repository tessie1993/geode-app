...
// this block is the meaning of ten user controls (Bloom, Posterize,
// Duotone, Saturation, Contrast, Gamma, Temperature, Solarize,
// Brightness/Intensity, Invert), and one slider has to mean one thing on every
// style that offers it. Flash dropped out of this list in wave three: the
// additive `uFlash * uBeat` term it drove was deleted with those uniforms.
//
...
vec3 grade(vec3 col) {
    ...
    // Wave three: the additive flash this term drove is gone - uFlash/uBeat no longer exist.
