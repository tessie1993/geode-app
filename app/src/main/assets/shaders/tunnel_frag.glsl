...
// motion: uEnergyRel -> travel speed (depth march rate), uBassRel -> tunnel
// radius via uBreath (continuous plate-style contraction, never a beat hit)
uniform float uTime;
...
vec2 view() {
    ...
    // Wave three: no beat-locked pulse or beat-response widening - the zoom is
    // steady and lets uBreath (continuous, bass-relative) carry the "breathing"
    // this used to fake with a beat envelope.
...
// Spectrum tunnel: ring brightness from bands, speed from relative energy.
void main() {
    vec2 uv = view();
    float angle = atan(uv.y, uv.x) / 6.2831 + 0.5;
    // uBreath (0.9..1.1, riding uBassRel/uBarOsc) breathes the radius gently
    // instead of the old instantaneous beat kick, so the tunnel narrows and
    // widens with the loudness of the passage rather than snapping per hit.
    float radius = length(uv) * uBreath + 0.0001;
    float depth = 0.3 / radius + uTime * (0.6 + uEnergyRel * 1.3);
    float band = aband(fract(angle + depth * 0.05));
    float rings = smoothstep(0.35, 0.9, fract(depth * 2.0));
    float wave = awave(fract(angle * 2.0));
    vec3 col = pal(depth * 0.15) * (0.15 + band * 1.6) * rings;
    col += pal(0.6) * wave * uTreble * 0.6;
    col *= smoothstep(1.8, 0.35, radius * uZoom);
