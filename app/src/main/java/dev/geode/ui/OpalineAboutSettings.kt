...
fun AboutSection() {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            CreativeButton(text = stringResource(R.string.about_licenses), onClick = { showLicenses = true })
            CreativeButton(
...
private fun LicensesDialog(onDismiss: () -> Unit) {
    ...
    AlertDialog(
        ...
        confirmButton = { CreativeButton(text = stringResource(R.string.action_close), onClick = onDismiss) },
    )
}
