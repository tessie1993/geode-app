...
void FlowField::step(const GeodeFeatureFrame& features, float dt, const SceneParams& p, const MotionField::State& motion) {
    if (!sim_.available()) return;
    ...
    emitters_.tick(features, simDt, sim_.aspect(), 0.0f, 1.0f, splats_, motion);
