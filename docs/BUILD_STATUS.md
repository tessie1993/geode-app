...
Snapshot: 848a374 2026-09-02   Branch: claude/repo-actions-6.4-8.2-cs6lz6   Last update: 2026-09-02
...
- 6.3: `AnimatableParams` is the one table of what a track may drive: scene fields under `scene.<field>` (labels from `ParamKeys`, ranges copied from the `CustomizeTabs` sliders so an animated value never leaves what the panel offers; int sliders round, choices clamp) and clip grade fields under `clip.<field>`. Palette indices are left out because their choice list is not a fixed table in the code read. The visualizer export folds programme-level scene tracks into `paramsAt` per frame on top of the take (or the live params); clip-scoped tracks are evaluated by the multi-clip export of 6.4. A new track starts with one key holding the current value so the curve has an origin. The custom curve's handles are dragged on a canvas drawn from the same `BezierCurve.eval` the export uses.

- 6.4: a transition is a property of the clip it opens (`Clip.transition`), from whatever ended before it on the same lane; a split tail never inherits it. It runs over the first `durationMs` of that clip against a freeze of the previous clip's last frame: one Media3 sequence plays items one after another and never holds two moving pictures at once, so a true overlap would have to extend both clips and move the cut. The frame crosses items through `TransitionFrameStore`: a `TransitionCaptureEffect` at the end of the previous clip's chain copies every output frame into its own texture on the GPU and reads it back once when the item's programs are released; the `GlTransitionEffect` uploads it as `uFrom`. Both programs latch the first `presentationTimeUs` they see as the clip's origin, so the result is the same whichever time base Media3 hands effects.
...
- UNVERIFIED: `androidx.media3.common.SimpleBasePlayer` (constructor with a `Looper`, `State.Builder`, `MediaItemData.Builder`, `handleSetMediaItems/AddMediaItems/RemoveMediaItems/MoveMediaItems/Seek/SetPlayWhenReady/Prepare/Stop/Release/SetRepeatMode/SetShuffleModeEnabled/SetVolume`, `invalidateState`) is used as named in the prompt; the jar was not opened.
