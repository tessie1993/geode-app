...
- `fractal_tunnel` (merged from the Fractal Tunnel branch after 4.9) is registered in the three places a fragment style now has: `SceneIds` + `SceneCapabilities.SHADER_SCENES`/`MARCHED_SCENES` for the UI and the Detail control, and the native `SceneRegistry` `kShaderIds` list, which is what builds it from `shaders/fractal_tunnel_frag.glsl`. It reads the shared contract only (`uSteps`, the touch block, `pal()`/`grade()`); no uniform was added.
...
- `fractal_tunnel_frag.glsl` likewise: written against the include contract and GLSL ES 3.00, not compiled or rendered (owner's instruction). First device run is the owner's.
...
- merge: Fractal Tunnel branch onto the native core (shader to assets/shaders, id in SceneIds/SceneCapabilities and the native kShaderIds, one built-in preset)
