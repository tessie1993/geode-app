...
**Maintained by hand.** It was generated by a `CustomizeSurfaceTest` that is
not in this repository, so a parameter added to the sources has to be added
here too (the `shapeMorph` row was).
...
| `shapeMorph` | Shape | `Shape morph` | `shapeMorph` | · |   |   |   |   |   |   |   |   |   |   |   |
