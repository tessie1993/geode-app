...
One raymarched `ShaderScene` style (`fractal_tunnel`, `engine/scenes/src/main/res/raw/fractal_tunnel_frag.glsl`), registered beside the other marched fragment styles in `SceneCapabilities.SHADER_SCENES` and `MARCHED_SCENES`, so it shares the uniform contract, the Detail march budget, the touch model, the palette and grade libraries, export and the visual-safety clamps without a renderer of its own.
